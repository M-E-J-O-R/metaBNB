...
const NavBar = () => {
    ...
    // this function helps display the mobile mode
    function handleMobileDisplay(){
        return {
            right: mobileDisplayer?'0px':'-100vw'
        }
    }

    return (<>
        <div className='nav-bar-wrapper'>
            <img className='metabnbIconn' src={metabnbIcon} alt="" />
            <img src={iconHamburger} className='mobile_displayer mobile_displayer-hide' onClick={()=>setMobileDisplayer(prev=>!prev)} alt=''/>
        

            <div style={handleMobileDisplay()} className='mobile_nav'>
